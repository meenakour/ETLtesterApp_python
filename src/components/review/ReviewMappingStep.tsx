...
import { MappingRowSelector } from '@/components/review/MappingRowSelector';
...
type Tab = 'mapping' | 'joins' | 'types' | 'fields';

export function ReviewMappingStep() {
  const { state, actions } = useAppState();
  const { requiredFieldsResolved, mappingRows, joinFilterRows } = useMappingData();
  ...
  const selectedFieldCount =
    state.selectedMappingRowIds === null ? mappingRows.length : state.selectedMappingRowIds.length;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex gap-2">
          ...
          {tabBtn('fields', `Select Fields (${selectedFieldCount}/${mappingRows.length})`)}
        </div>
      ...
      {tab === 'fields' && (
        <div className="space-y-4">
          <MappingRowSelector mappingRows={mappingRows} />
        </div>
      )}
